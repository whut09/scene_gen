...
export const decisionFlowTemplate: HtmlTemplateDefinition = {
  ...
  renderHtml: ({ scene, width, height }) => {
    ...
    const css =
      ...
      '.df-main h1{font-size:' + titleSize + 'px;max-width:940px;flex:0 0 auto}.df-flow{position:relative;left:auto;right:auto;top:auto;bottom:auto;display:grid;grid-template-rows:repeat(' + rowCount + ',minmax(132px,1fr));align-items:center;gap:14px;flex:1;min-height:0;margin-top:28px}' +
      ...
      '.df-node{position:relative;width:48%;height:auto;min-height:132px;max-height:none;padding:20px 24px;display:grid;grid-template-columns:54px minmax(0,1fr);gap:16px;align-items:center;align-self:center;background:rgba(255,255,255,.15);border:2px solid rgba(255,255,255,.28);box-shadow:0 20px 54px rgba(0,30,86,.18);animation:df-in .55s both;overflow:visible}' +
      ...
      '.df-node b{display:block;font-size:29px;line-height:1.12;overflow-wrap:anywhere}.df-node p{font-size:22px;line-height:1.34;margin-top:7px;overflow-wrap:anywhere;word-break:break-word}' +
