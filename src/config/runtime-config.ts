...
export const runtimeConfigSchema = z.object({
  ...
  tts: z.object({
    ...
    indextts: z.object({ root: z.string().min(1), modelDir: z.string().min(1), refAudio: z.string().min(1), glossary: z.string().min(1), python: z.string().min(1), workerScript: z.string().min(1), speakerCheckScript: z.string().min(1), concurrency: positiveInteger, timeoutMs: positiveInteger, readyTimeoutMs: positiveInteger, minimumReferenceSeconds: positiveNumber, minimumSpeakerSimilarity: z.number().min(0).max(1), seed: z.number().int(), topP: z.number().positive().max(1), topK: positiveInteger, temperature: z.number().positive().max(2), repetitionPenalty: positiveNumber, tempo: z.number().min(0.5).max(2), loudnessLufs: z.number().finite().max(0), truePeakDb: z.number().finite().max(0) }),
  ...
  asr: z.object({
    disabled: z.boolean(), provider: z.enum(["whisper", "sensevoice", "funasr", "mock"]).default("whisper"), python: z.string().optional(), model: z.string(), language: z.string(), languageConfidenceMin: z.number().min(0).max(1).default(0.5), titleCoverageMin: z.number().min(0).max(1), sceneConfidenceMin: z.number().min(0).max(1).default(0.65), sceneTokenCoverageMin: z.number().min(0).max(1).default(0.78), sceneTokenPrecisionMin: z.number().min(0).max(1).default(0.75), entityRecallMin: z.number().min(0).max(1).default(0.8), semanticConfidenceMin: z.number().min(0).max(1).default(0.84), boundaryLeakMin: z.number().min(0).max(1).default(0.55), endingRecallMin: z.number().min(0).max(1).default(0.62),
    pronunciation: z.object({
...
function providerValue(value: string | undefined, fallback: "indextts" | "nvidia" | "azure" | "cloudflare-melotts" | "edge" | "openai" | "f5" | "local" | "mock") {
  return value === "indextts" || value === "nvidia" || value === "azure" || value === "cloudflare-melotts" || value === "edge" || value === "openai" || value === "f5" || value === "local" || value === "mock" ? value : fallback;
}
...
export function buildRuntimeConfig(env: NodeJS.ProcessEnv = process.env, profile = stringValue(env, "SCENE_GEN_PROFILE", "custom") ?? "custom"): RuntimeConfig {
  ...
  const config = runtimeConfigSchema.parse({
    ...
    tts: {
      provider: profile === "production" || profile === "indextts-local"
        ? "indextts"
        : providerValue(stringValue(env, "TTS_PROVIDER"), profile === "local-f5" ? "f5" : "openai"),
      ...
      narrationIdentity: {
        ...
        rateTolerance: numberValue(env, "TTS_EXPECTED_RATE_TOLERANCE", 0.02),
      },
      failFast: booleanValue(env, "TTS_FAIL_FAST"), durationPolicy: stringValue(env, "TTS_DURATION_POLICY", "natural") === "fit" ? "fit" : "natural", fitTarget: stringValue(env, "TTS_FIT_TARGET") !== "0", forceRebuild: booleanValue(env, "TTS_FORCE_REBUILD"), leadingSilenceSeconds: numberValue(env, "TTS_LEADING_SILENCE_SECONDS", 1.8), fetchTimeoutMs: numberValue(env, "TTS_FETCH_TIMEOUT_MS", 180_000), minTempo: numberValue(env, "TTS_MIN_TEMPO", 0.9), maxTempo: numberValue(env, "TTS_MAX_TEMPO", 1.22), preprocessConcurrency: numberValue(env, "TTS_PREPROCESS_CONCURRENCY", 4), ffmpegConcurrency: numberValue(env, "TTS_FFMPEG_CONCURRENCY", 2),
      azure: {
      ...
      nvidia: { apiKey: stringValue(env, "NVIDIA_API_KEY"), endpoint: stringValue(env, "NVIDIA_TTS_ENDPOINT", "grpc.nvcf.nvidia.com:443")!, functionId: stringValue(env, "NVIDIA_TTS_FUNCTION_ID", "877104f7-e885-42b9-8de8-f6e4c6303969")!, model: stringValue(env, "NVIDIA_TTS_MODEL", "magpie-tts-multilingual")!, voice: stringValue(env, "NVIDIA_TTS_VOICE", "Magpie-Multilingual.ZH-CN.Siwei")!, language: stringValue(env, "NVIDIA_TTS_LANGUAGE", "zh-CN")!, sampleRateHz: numberValue(env, "NVIDIA_TTS_SAMPLE_RATE_HZ", 44100), speed: numberValue(env, "NVIDIA_TTS_SPEED", 1.25), concurrency: numberValue(env, "NVIDIA_TTS_CONCURRENCY", 1), timeoutMs: numberValue(env, "NVIDIA_TTS_TIMEOUT_MS", 180000), readyTimeoutMs: numberValue(env, "NVIDIA_TTS_READY_TIMEOUT_MS", 30000), python: stringValue(env, "NVIDIA_TTS_PYTHON", process.platform === "win32" ? "python" : "python3")!, workerScript: stringValue(env, "NVIDIA_TTS_WORKER_SCRIPT", fromRoot("scripts", "nvidia-tts-worker.py"))!, transport: stringValue(env, "NVIDIA_TTS_TRANSPORT", "grpc") === "http" ? "http" : stringValue(env, "NVIDIA_TTS_TRANSPORT", "grpc") === "grpc" ? "grpc" : "auto" },
      indextts: { root: stringValue(env, "INDEXTTS_ROOT", fromRoot("..", "models", "index-tts"))!, modelDir: stringValue(env, "INDEXTTS_MODEL_DIR", fromRoot("..", "models", "index-tts", "checkpoints"))!, refAudio: stringValue(env, "INDEXTTS_REF_AUDIO", fromRoot("..", "models", "index-tts", "local", "mandarin-siwei-reference.wav"))!, glossary: stringValue(env, "INDEXTTS_GLOSSARY", fromRoot("config", "tts", "indextts-glossary.yaml"))!, python: stringValue(env, "INDEXTTS_PYTHON", "python")!, workerScript: stringValue(env, "INDEXTTS_WORKER_SCRIPT", fromRoot("scripts", "indextts-worker.py"))!, speakerCheckScript: stringValue(env, "INDEXTTS_SPEAKER_CHECK_SCRIPT", fromRoot("scripts", "indextts-speaker-check.py"))!, concurrency: numberValue(env, "INDEXTTS_CONCURRENCY", 1), timeoutMs: numberValue(env, "INDEXTTS_TIMEOUT_MS", 600000), readyTimeoutMs: numberValue(env, "INDEXTTS_READY_TIMEOUT_MS", 180000), minimumReferenceSeconds: numberValue(env, "INDEXTTS_MIN_REFERENCE_SECONDS", 5), minimumSpeakerSimilarity: numberValue(env, "INDEXTTS_MIN_SPEAKER_SIMILARITY", 0.8), seed: numberValue(env, "INDEXTTS_SEED", 20260721), topP: numberValue(env, "INDEXTTS_TOP_P", 0.65), topK: numberValue(env, "INDEXTTS_TOP_K", 20), temperature: numberValue(env, "INDEXTTS_TEMPERATURE", 0.65), repetitionPenalty: numberValue(env, "INDEXTTS_REPETITION_PENALTY", 10), tempo: numberValue(env, "INDEXTTS_TEMPO", 1.22), loudnessLufs: numberValue(env, "INDEXTTS_LOUDNESS_LUFS", -19), truePeakDb: numberValue(env, "INDEXTTS_TRUE_PEAK_DB", -2) },
    ...
    asr: {
      disabled: booleanValue(env, "ASR_DISABLED"), provider: ["sensevoice", "funasr", "mock"].includes(stringValue(env, "ASR_PROVIDER", "whisper")!) ? stringValue(env, "ASR_PROVIDER") as "sensevoice" | "funasr" | "mock" : "whisper", python: stringValue(env, "ASR_PYTHON"), model: stringValue(env, "ASR_MODEL", "openai/whisper-tiny")!, language: stringValue(env, "ASR_LANGUAGE", "chinese")!, languageConfidenceMin: numberValue(env, "ASR_LANGUAGE_CONFIDENCE_MIN", 0.5), titleCoverageMin: numberValue(env, "ASR_TITLE_COVERAGE_MIN", 0.58), sceneConfidenceMin: numberValue(env, "ASR_SCENE_CONFIDENCE_MIN", 0.65), sceneTokenCoverageMin: numberValue(env, "ASR_SCENE_TOKEN_COVERAGE_MIN", 0.78), sceneTokenPrecisionMin: numberValue(env, "ASR_SCENE_TOKEN_PRECISION_MIN", 0.75), entityRecallMin: numberValue(env, "ASR_ENTITY_RECALL_MIN", 0.8), semanticConfidenceMin: numberValue(env, "ASR_SEMANTIC_CONFIDENCE_MIN", 0.84), boundaryLeakMin: numberValue(env, "ASR_BOUNDARY_LEAK_MIN", 0.55), endingRecallMin: numberValue(env, "ASR_ENDING_RECALL_MIN", 0.62),
      pronunciation: {
