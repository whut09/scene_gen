...
export async function publishAgentRun(input: {
  ...
  const publish = await runStage({
    ...
    describe: (value) => ({ outputs: { outputPath: value.outputPath, reportPath: value.reportPath, markdownPath: value.markdownPath, productionReportPath: value.productionReportPath, templateOutcomeFile: value.templateLearning.filePath }, metrics: { passed: value.passed, templateOutcomesRecorded: value.templateLearning.recorded, providerOutcomesRecorded: value.providerHistory.recorded } }),
  });
  await input.journal.setArtifacts({ outputPath: publish.value.outputPath, qualityReport: publish.value.reportPath, qualityReportMarkdown: publish.value.markdownPath, productionReport: publish.value.productionReportPath });
