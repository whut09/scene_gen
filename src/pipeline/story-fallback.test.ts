...
test("repository asset evidence replaces the middle proof scene and keeps narration aligned", () => {
  ...
  assert.match(withAssets.scenes[2]?.headline ?? "", /项目工作流.*效果图/);
...
test("GPT Image report keeps complete connected narration near sixty seconds", () => {
  ...
  assert.equal(project.narrationSegments?.every((segment) => /[。！？]$/u.test(segment.text)), true);
  assert.equal(project.narrationSegments?.[0]?.text.length! < 64, true);
  ...
  assert.match(compacted.narration, /正式名称、价格、公开时间和最终能力/);
});
