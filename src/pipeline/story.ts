...
function repositoryProfile(item: HotItem): RepositoryProfile {
  ...
  if (/^free-for-dev$/i.test(name)) {
    return {
      ...
      capability: "按数据库、托管、监控、邮件、存储、测试和开发工具分类整理免费层服务，减少逐个平台查找和比较的时间",
      ...
      problemPoints: [
        ...
        "free-for-dev 把开发者可用的免费层服务按类别整理成目录，帮助用户更快找到适合当前项目的候选。",
      ...
      narration: [
        "开源项目推荐：free-for-dev。它把开发者常用的免费服务按类别整理，解决做原型时逐个平台查找和比较的麻烦。",
