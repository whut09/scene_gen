import { calculateProviderStats, providerCostMetric, readProviderOutcomes, type ProviderSelectionContext } from "./provider-stats";
import type { ProviderCandidateDecision, ProviderCapability, ProviderDescriptor, ProviderSelectionAudit } from "./types";
...
function definitions(): ProviderDefinition[] { return [
  ...
  { id: "f5", name: "F5-TTS", capability: "tts", enabled: configured("F5_TTS_VENV", "F5_TTS_PYTHON"), local: true, priorQuality: 0.86, priorCost: 0, priorLatency: 0.7, priorPronunciationAccuracy: 0.93, supportsPortrait: true, commercialUse: true, reason: "Requires F5_TTS_VENV or F5_TTS_PYTHON" },
  { id: "azure-speech", name: "Azure Speech TTS", capability: "tts", enabled: configured("AZURE_SPEECH_KEY") && configured("AZURE_SPEECH_REGION", "AZURE_SPEECH_ENDPOINT"), local: false, priorQuality: 0.94, priorCost: 0, priorLatency: 0.38, priorPronunciationAccuracy: 0.96, supportsPortrait: true, commercialUse: true, reason: "Requires AZURE_SPEECH_KEY and AZURE_SPEECH_REGION or AZURE_SPEECH_ENDPOINT" },
  { id: "openai-tts", name: "OpenAI-compatible TTS", capability: "tts", enabled: configured("OPENAI_TTS_API_KEY", "OPENAI_API_KEY"), local: false, priorQuality: 0.9, priorCost: 0.2, priorLatency: 0.35, priorPronunciationAccuracy: 0.9, supportsPortrait: true, commercialUse: true, reason: "Requires OPENAI_TTS_API_KEY or OPENAI_API_KEY" },
  { id: "local-tts", name: "Operating-system TTS", capability: "tts", enabled: process.platform === "win32", local: true, priorQuality: 0.55, priorCost: 0, priorLatency: 0.2, priorPronunciationAccuracy: 0.62, supportsPortrait: true, commercialUse: true, fallbackOnly: true, reason: "Windows System.Speech fallback" },
