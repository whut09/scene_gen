...
Draft 不再直接要求模型一次性生成完整项目，而是执行两个独立调用。新闻和开源项目默认生成四屏短版，技术文章默认生成五屏解释版。

## 规划阶段

规划模型只返回候选的叙事角度、标题、预计时长、逐屏 purpose/focus 和事实 `claimIds`，不生成完整旁白或视觉字段。候选数量由 `STORY_PLAN_CANDIDATES` 控制：
...
`story-planner.ts` 在本地检查未知事实引用、内容类型对应的场景顺序、重复 focus、无法可视化的文本长度、预计时长和标题高风险谓词。被否决方案不会交给另一个 LLM 重新打分。

默认内容预算：新闻 32～45 秒，开源项目 40～55 秒，技术文章 50～70 秒。短版在前 6 秒给出核心价值，并在视频过半前覆盖至少 70% 的关键事实。
