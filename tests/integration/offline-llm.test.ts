...
test("fixed article is directed by a mocked OpenAI-compatible response", async () => {
  ...
  const response = {
    ...
    sections: [
      { narration: "本次更新先说明离线质量门禁。", kicker: "工程更新", subhead: "固定文章离线集成测试", keywords: ["离线", "质量", "视频"] },
      { narration: "第一步验证固定文章事实。", headline: "固定事实进入结构化脚本", summary: "文章明确描述模板、缓存和门禁改进。", metrics: [{ label: "模式", value: "离线" }, { label: "输入", value: "固定文章" }], points: ["不访问真实模型", "保留文章事实"] },
      { narration: "第二步比较三个验证层。", headline: "三个验证层覆盖关键边界", bars: [{ label: "单元", value: 80, detail: "纯函数与规则" }, { label: "媒体", value: 70, detail: "音视频门禁" }, { label: "模板", value: 60, detail: "截图与安全区" }] },
      { narration: "第三步串联生成检查发布。", headline: "离线流程串联关键阶段", steps: [{ label: "生成", detail: "模拟模型响应" }, { label: "检查", detail: "运行质量门禁" }, { label: "发布", detail: "输出可审计结果" }] },
      { narration: "最终结论是离线测试可以稳定复现。", headline: "离线验证降低回归风险", bullets: ["固定输入便于复现", "模拟响应不消耗 API"] },
    ],
  };
